...
import com.test.CNHi.pages.EVOFarmHomePage;
import com.test.CNHi.testdata.D_EVOFleetApplicationLoginPage;
import com.test.CNHi.testdata.D_EVOFleetHomePage;

public class  CNHiFarmAPPVehicleStatusTest extends FW_BaseTestCase {

	private static Logger logger = LogManager.getLogger(CNHiFarmAPPVehicleStatusTest.class.getName());
	...
	EVOFarmHomePage evoFarmHomePage=PageFactory.initElements(d, EVOFarmHomePage.class);
	...
	public void test () {
		...
		evoFarmHomePage.clickApplicationButton();
		evoFarmHomePage.clickFleetIcon();
		evoFarmHomePage.getGlobalStatusOfVehicles();
	}
	...
	public boolean verify() {
		...
		expectedStatus.setT(evoFarmHomePage.geExpectedStatusFromExcel());
		actualStatus.setT(evoFarmHomePage.getActualStatusOfVechicle());

		if (VerifySafely("Vehicle Status expected.", expectedStatus, actualStatus)) {
			logger.info("Vehicle Status is as expected [Expected] " + expectedStatus.getT() + "   [Actual] " +  actualStatus.getT());
			...
			logger.info("Vehicle Status is not as expected [Expected] " + expectedStatus.getT() + "   [Actual] " +  actualStatus.getT());
