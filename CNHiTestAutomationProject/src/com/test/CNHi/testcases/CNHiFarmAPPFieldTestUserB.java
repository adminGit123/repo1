...
import com.test.CNHi.pages.EVOFarmApplicationLoginPage;
...
public class CNHiFarmAPPFieldTestUserB extends FW_BaseTestCase {

	private static Logger logger = LogManager.getLogger(CNHiFarmAPPFieldTestUserB.class.getName());
	...
	public void setup () {
		...
		evoFarmApplicationLoginPage.getInputData("D_EVOFleetApplicationLoginPage.csv", 1);
	...
	public boolean verify() {
		...
		expectedStatus.setT(evoFarmFieldPage.geExpectedFieldsFromExcel());
		actualStatus.setT(evoFarmFieldPage.getActualGFFValue());

		if (VerifySafely("Fields Status expected.", expectedStatus, actualStatus)) {
			logger.info("Fields Status is as expected [Expected] " + expectedStatus.getT() + "   [Actual] " +  actualStatus.getT());
			fw_WebDriverFunctions.captureScreenshot(imageFolder + "9.png");
			fw_ExtentReporter.extentMessage(logStatus.PASS.toString(), "Fields Status is as expected [Expected] " + expectedStatus.getT() + "   [Actual] " +  actualStatus.getT(), relativePath + "9.png");
			return true;

		} else {
			logger.info("Fields Status is not as expected [Expected] " + expectedStatus.getT() + "   [Actual] " +  actualStatus.getT());
			fw_WebDriverFunctions.captureScreenshot(imageFolder + "10.png");
			fw_ExtentReporter.extentMessage(logStatus.FAIL.toString(), "Fields Status is as expected [Expected]  " + expectedStatus.getT() + "   [Actual] " +  actualStatus.getT(), relativePath + "10.png");
			return false;
		}
